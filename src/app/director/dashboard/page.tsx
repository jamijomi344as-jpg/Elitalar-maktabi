...
export default function DirectorDashboard() {
  ...
  const [workloadForm, setWorkloadForm] = useState<{ class_names: string[], subject: string, teacher_id: string, hours: number, group_type: string }>({ class_names: [], subject: "", teacher_id: "", hours: 2, group_type: "Barchasi" });
  const [showClassDropdown, setShowClassDropdown] = useState(false); // Multi-select uchun
  ...
  ].sort(); // "Kelajak soati" va "Sinf soati" ro'yxatdan olib tashlandi, avtomat qilinadi!
  ...
  const handleAddWorkload = () => {
    ...
    setWorkloads(updated); localStorage.setItem('elita_workloads', JSON.stringify(updated));
    setWorkloadForm({...workloadForm, class_names: [], teacher_id: "", subject: "", hours: 2, group_type: "Barchasi"}); 
    showToast("Yuklamalar muvaffaqiyatli qo'shildi!"); setShowClassDropdown(false);
  };
  ...
  const handleAutoGenerate = async () => {
    setConfirmDialog({
      ...
      onConfirm: async () => {
        setConfirmDialog(null); setIsGenerating(true);
        ...
        if(insertData.length > 0) {
          await supabase.from('timetable').insert(insertData); showToast("Algoritm jadvalni muvaffaqiyatli tuzdi!", "success");
        } else { showToast("Xatolik! Jadval tuza olmadim.", "error"); }
  ...
  const handleSaveLesson = async () => {
    ...
    if (currentCellLessons.length > 0) {
      if (lessonForm.group_type === 'Barchasi' || currentCellLessons.some(c => c.group_type === 'Barchasi')) {
         setConflictWarning(`🔴 XATO: Bu vaqtga boshqa guruh qo'shib bo'lmaydi.`); return;
      }
    }
    ...
    if(!error) { showToast("Dars saqlandi!"); setLessonForm({ subject: "", teacher_id: "", room: "", group_type: "Barchasi" }); fetchData(); } else showToast(error.message, "error");
  };

  const deleteLessonItem = async (id: string) => {
    await supabase.from('timetable').delete().eq('id', id); showToast("Dars o'chirildi!"); fetchData();
  };
  ...
  const handleAddTeacher = async () => {
    ...
    if (!error) { showToast(`Qo'shildi! ID: ${uniqueId}`); setShowTeacherModal(false); setNewPerson({ fullName: "", subject: "", homeroom: "", className: "" }); fetchData(); }
  };
  const handleUpdateTeacher = async () => {
    ...
    if(!error) { showToast("Saqlandi!"); setShowEditTeacherModal(false); fetchData(); }
  };
  ...
  const handleAddStudent = async () => {
    ...
    await supabase.from('profiles').insert([{ id, role: 'student', full_name: newPerson.fullName, class_name: newPerson.className, password: pass }]);
    ...
    <div className="flex h-screen bg-slate-50 font-sans overflow-hidden relative">
      ...
      {toast && (
        <div className={`fixed top-6 right-6 z-[100] px-6 py-4 rounded-2xl font-bold text-white shadow-2xl flex items-center gap-3 animate-in slide-in-from-right-8 ${toast.type === 'success' ? 'bg-emerald-500' : 'bg-red-500'}`}>
          {toast.type === 'success' ? <CheckCircle2 className="w-6 h-6"/> : <AlertTriangle className="w-6 h-6"/>}{toast.message}
        </div>
      )}
      ...
      {confirmDialog && (
        <div className="fixed inset-0 z-[100] bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4 animate-in fade-in">
          <div className="bg-white p-8 rounded-3xl shadow-2xl max-w-sm w-full text-center animate-in zoom-in-95">
            <div className="w-20 h-20 bg-red-100 text-red-500 rounded-full flex items-center justify-center mx-auto mb-6"><AlertTriangle className="w-10 h-10"/></div>
            <h3 className="text-2xl font-black text-slate-900 mb-2">Tasdiqlang!</h3><p className="text-slate-500 font-medium mb-8 leading-relaxed">{confirmDialog.message}</p>
            <div className="flex gap-3"><button onClick={() => setConfirmDialog(null)} className="flex-1 py-4 bg-slate-100 text-slate-600 rounded-2xl font-bold hover:bg-slate-200 transition-colors">Yo'q, qaytish</button><button onClick={confirmDialog.onConfirm} className="flex-1 py-4 bg-red-500 text-white rounded-2xl font-bold shadow-lg hover:bg-red-600 transition-colors">Ha, bajarilsin</button></div>
          </div>
        </div>
      )}
      ...
      <aside className="w-72 bg-slate-950 text-slate-400 p-6 hidden md:flex flex-col border-r border-slate-800 relative z-10">
        <div className="flex items-center gap-3 mb-12 px-2"><div className="w-10 h-10 bg-indigo-600 rounded-2xl flex items-center justify-center text-white font-black shadow-lg shadow-indigo-500/20">E</div><span className="text-2xl font-black text-white tracking-tighter italic">ELITA</span></div>
      ...
      <main className="flex-1 overflow-y-auto p-8 lg:p-12 relative z-0">
        <header className="flex justify-between items-center mb-10"><div><h1 className="text-4xl font-black text-slate-900 tracking-tight">Direktor Paneli</h1><p className="text-slate-500 font-medium mt-1">Elita Meta-Education tizimi boshqaruvi</p></div></header>
              ...
              <div className="space-y-8">
                ...
                <div className="bg-white rounded-[3rem] shadow-sm border border-slate-100 overflow-hidden">
                  ...
                  <div className="p-6 space-y-4 max-h-[600px] overflow-y-auto">
                    {feedbacks.length === 0 ? <div className="text-center p-10 text-slate-400 font-bold border-2 border-dashed border-slate-100 rounded-3xl">Hozircha murojaatlar yo'q.</div> : feedbacks.map(f => (
              ...
              <div className="bg-white rounded-[3rem] shadow-sm border border-slate-100 overflow-hidden min-h-[700px] flex flex-col animate-in fade-in">
                <div className="p-8 border-b border-slate-50 flex flex-col xl:flex-row justify-between items-start xl:items-center gap-6">
                   ...
                   <div className="flex flex-wrap gap-4 items-center">
                     <button onClick={() => setShowWorkloadModal(true)} className="px-6 py-3 border border-indigo-100 text-indigo-700 bg-indigo-50 font-black rounded-2xl shadow-sm hover:bg-indigo-100 transition-all flex items-center gap-2"><FileText className="w-5 h-5"/> Yuklamalarni Kiritish</button>
                     <button onClick={handleAutoGenerate} disabled={isGenerating} className="px-6 py-3 bg-gradient-to-r from-indigo-500 to-blue-600 text-white font-black rounded-2xl shadow-lg hover:scale-105 transition-transform flex items-center gap-2 disabled:opacity-50"><Wand2 className="w-5 h-5"/> {isGenerating ? "Jadval tuzilmoqda..." : "Avtomatik Tuzish"}</button>
                     <div className="flex bg-slate-100 p-1.5 rounded-2xl">
                       {classes.map(c => (
                         <button key={c.name} onClick={() => setSelectedClassForTimetable(c.name)} className={`px-5 py-2.5 rounded-xl font-black text-sm transition-all ${selectedClassForTimetable === c.name ? 'bg-white text-indigo-600 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}>{c.name}</button>
                       ))}
                     </div>
                ...
                {!selectedClassForTimetable ? (
                  <div className="flex-1 flex flex-col items-center justify-center p-20 text-slate-300"><Calendar className="w-20 h-20 mb-4 opacity-20"/><p className="text-xl font-bold italic">Sinfni tanlang</p></div>
                ) : (
                  <div className="flex-1 overflow-x-auto p-8 bg-slate-50/50">
                    <table className="w-full border-collapse bg-white shadow-sm rounded-2xl overflow-hidden border border-slate-100">
                      <thead>
                        <tr><th className="p-4 bg-slate-50 border-b border-r border-slate-100 w-20"><Clock className="w-5 h-5 mx-auto text-slate-300"/></th>{days.map(d => <th key={d} className="p-4 bg-slate-50 border-b border-slate-100 text-slate-400 font-black uppercase text-xs tracking-widest">{d}shanba</th>)}</tr>
                      </thead>
                      <tbody>
                            ...
                            {days.map(day => {
                              ...
                              return (
                                <td key={day+num} onClick={() => { setCurrentCell({day, lesson: num}); setLessonForm({subject: cellLessons[0]?.subject || "", teacher_id: cellLessons[0]?.teacher_id || "", room: cellLessons[0]?.room || "", group_type: "Barchasi"}); setConflictWarning(null); setShowLessonModal(true); }} className={`p-2 border-b border-slate-100 h-32 w-44 cursor-pointer transition-all hover:bg-indigo-50/50 group relative align-top`}>
                                  {cellLessons.length === 0 && <div className="h-full flex items-center justify-center opacity-0 group-hover:opacity-100"><PlusCircle className="text-indigo-300 w-6 h-6"/></div>}
                                    ...
                                    <div className="flex h-full gap-1">
                                      {cellLessons.map(lesson => (
                                        <div key={lesson.id} className="flex-1 h-full flex flex-col justify-center bg-indigo-50/80 rounded-xl p-2 border border-indigo-100 relative group/item">
                                          ...
                                          {lesson.group_type !== 'Barchasi' && <span className="text-[8px] bg-indigo-600 text-white px-1.5 py-0.5 rounded mt-1 self-start inline-block">{lesson.group_type}</span>}
                                          ...
                                          <button onClick={(e) => { e.stopPropagation(); deleteLessonItem(lesson.id); }} className="absolute -top-2 -right-2 bg-red-500 text-white rounded-full p-1 opacity-0 group-hover/item:opacity-100 shadow-md"><X className="w-3 h-3"/></button>
                                        </div>
                                      ))}
                                    </div>
                                  )}
        ...
        <div className="fixed inset-0 bg-slate-950/80 backdrop-blur-md z-50 flex items-center justify-center p-4 animate-in fade-in" onClick={() => setShowWorkloadModal(false)}>
           <div className="bg-white rounded-[3rem] w-full max-w-5xl overflow-hidden shadow-2xl animate-in zoom-in-95 flex flex-col max-h-[90vh]" onClick={e => { e.stopPropagation(); setShowClassDropdown(false); }}>
              <div className="p-8 bg-indigo-50 border-b border-indigo-100 flex justify-between items-center">
                 <div><h3 className="text-2xl font-black text-indigo-900">O'quv Rejasi (Yuklamalar)</h3><p className="text-indigo-600 font-bold text-sm mt-1">Ko'p sinf tanlash imkoniyati bilan.</p></div>
              ...
              <div className="p-6 bg-slate-50 border-b border-slate-100 flex flex-wrap gap-4 items-end">
                ...
                <div className="relative flex-1 min-w-[150px]" onClick={e => e.stopPropagation()}>
                  ...
                  <div className="w-full p-3 rounded-xl border border-slate-200 bg-white cursor-pointer font-bold flex justify-between items-center text-sm" onClick={() => setShowClassDropdown(!showClassDropdown)}>
                    <span className="truncate pr-2">{workloadForm.class_names.length > 0 ? `${workloadForm.class_names.length} ta sinf tanlandi` : "Sinf..."}</span><ChevronDown className="w-4 h-4 flex-shrink-0" />
                  </div>
                  {showClassDropdown && (
                ...
                <div className="flex-[1.5] min-w-[150px]"><label className="text-[10px] font-black text-slate-400 uppercase mb-1 block">Fan</label><select className="w-full p-3 rounded-xl border border-slate-200 outline-none font-bold text-sm" value={workloadForm.subject} onChange={e=>setWorkloadForm({...workloadForm, subject: e.target.value, teacher_id: ""})}><option value="">Fan...</option>{subjectsBase.map(s=><option key={s} value={s}>{s}</option>)}</select></div>
                
                <div className="flex-1 min-w-[140px]"><label className="text-[10px] font-black text-slate-400 uppercase mb-1 block">Guruh</label><select className="w-full p-3 rounded-xl border border-slate-200 outline-none font-bold text-sm" value={workloadForm.group_type} onChange={e=>setWorkloadForm({...workloadForm, group_type: e.target.value})}>{groupTypes.map(g=><option key={g} value={g}>{g}</option>)}</select></div>

                <div className="flex-[2] min-w-[180px]"><label className="text-[10px] font-black text-slate-400 uppercase mb-1 block">O'qituvchi</label><select className="w-full p-3 rounded-xl border border-slate-200 outline-none font-bold text-sm" value={workloadForm.teacher_id} onChange={e=>setWorkloadForm({...workloadForm, teacher_id: e.target.value})}><option value="">Kim o'tadi...</option>{teachers.filter(t => !workloadForm.subject || t.bio === workloadForm.subject).map(t=><option key={t.id} value={t.id}>{t.full_name}</option>)}</select></div>
                
                <div className="w-24"><label className="text-[10px] font-black text-slate-400 uppercase mb-1 block">Soat</label><input type="number" min="1" max="6" className="w-full p-3 rounded-xl border border-slate-200 outline-none font-bold text-center text-sm" value={workloadForm.hours} onChange={e=>setWorkloadForm({...workloadForm, hours: Number(e.target.value)})} /></div>
                
                <button onClick={handleAddWorkload} className="p-3 px-6 bg-indigo-600 text-white rounded-xl hover:bg-indigo-700 shadow-md font-black flex items-center justify-center gap-2"><PlusCircle className="w-5 h-5"/></button>
              </div>

              <div className="flex-1 overflow-y-auto p-6 bg-white">
                {workloads.length === 0 ? <div className="text-center p-10 text-slate-400 font-bold border-2 border-dashed border-slate-100 rounded-3xl">Hali hech qanday dars yuklamasi kiritilmagan. Yuqoridan qo'shing.</div> : <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">{workloads.map(w => <div key={w.id} className="flex items-center justify-between p-4 bg-slate-50 border border-slate-100 rounded-2xl"><div className="flex items-center gap-4"><span className="w-12 h-12 bg-indigo-100 text-indigo-700 rounded-xl flex items-center justify-center font-black text-sm">{w.class_name}</span><div><p className="font-black text-slate-900 text-sm">{w.subject} <span className="text-indigo-600 ml-1 text-xs">({w.group_type})</span></p><p className="text-xs font-bold text-slate-500 mt-1">{teachers.find(t=>t.id===w.teacher_id)?.full_name} | {w.hours} soat</p></div></div><button onClick={() => handleDeleteWorkload(w.id)} className="p-2.5 bg-red-50 text-red-500 hover:bg-red-500 hover:text-white rounded-xl transition-colors"><Trash2 className="w-4 h-4"/></button></div>)}</div>}
              </div>
           </div>
        </div>
      )}
      ...
      {showLessonModal && currentCell && (
        <div className="fixed inset-0 bg-slate-950/80 backdrop-blur-md z-50 flex items-center justify-center p-4" onClick={() => setShowLessonModal(false)}>
           <div className="bg-white rounded-[3rem] w-full max-w-md overflow-hidden shadow-2xl" onClick={e => e.stopPropagation()}>
              <div className="p-8 bg-indigo-50 border-b border-indigo-100 flex justify-between items-center"><div><h3 className="text-xl font-black text-indigo-900">{currentCell.day}, {currentCell.lesson}-soat</h3><p className="text-indigo-600 font-bold text-xs uppercase tracking-widest mt-1">{selectedClassForTimetable}</p></div><button onClick={() => setShowLessonModal(false)} className="bg-white p-2 rounded-full text-indigo-400 hover:text-indigo-600"><X/></button></div>
              <div className="p-8 space-y-4">
                 {conflictWarning && <div className="bg-red-50 border border-red-200 text-red-600 p-4 rounded-2xl text-sm font-bold flex items-start gap-3"><AlertTriangle className="w-6 h-6 flex-shrink-0 mt-0.5" /><div>{conflictWarning}</div></div>}
                 
                 <select className="w-full p-4 bg-slate-50 border border-slate-200 rounded-2xl font-bold outline-none focus:border-indigo-500" value={lessonForm.group_type} onChange={e => setLessonForm({...lessonForm, group_type: e.target.value})}>{groupTypes.map(g => <option key={g} value={g}>{g}</option>)}</select>
      ...
      {/* BOSHQA BARCHA MODALLAR (O'qituvchi, O'quvchi qo'shish va Tahrirlash) */}
      {replyModal && <div className="fixed inset-0 bg-slate-950/80 z-50 flex items-center justify-center p-4" onClick={() => setReplyModal(null)}><div className="bg-white rounded-[3rem] w-full max-w-md shadow-2xl p-8" onClick={e=>e.stopPropagation()}><h3 className="text-xl font-black mb-4">Javob Yozish</h3><textarea rows={4} className="w-full p-4 bg-slate-50 rounded-2xl font-bold outline-none mb-4 border" value={replyText} onChange={e=>setReplyText(e.target.value)}></textarea><button onClick={handleSendReply} className="w-full py-4 bg-indigo-600 text-white rounded-2xl font-black">YUBORISH</button></div></div>}
      {showTeacherModal && <div className="fixed inset-0 bg-slate-950/80 z-50 flex items-center justify-center p-4" onClick={() => setShowTeacherModal(false)}><div className="bg-white rounded-[3rem] w-full max-w-md shadow-2xl p-8" onClick={e=>e.stopPropagation()}><div className="flex justify-between mb-6"><h3 className="text-xl font-black">Yangi O'qituvchi</h3><button onClick={() => setShowTeacherModal(false)}><X/></button></div><input type="text" placeholder="F.I.SH" className="w-full p-4 bg-slate-100 rounded-2xl font-bold outline-none mb-4" value={newPerson.fullName} onChange={e=>setNewPerson({...newPerson, fullName: e.target.value})} /><select className="w-full p-4 bg-slate-100 rounded-2xl font-bold outline-none mb-4" value={newPerson.subject} onChange={e=>setNewPerson({...newPerson, subject: e.target.value})}><option value="">Fan...</option>{subjectsBase.map(s=><option key={s} value={s}>{s}</option>)}</select><select className="w-full p-4 bg-blue-50 text-blue-600 rounded-2xl font-bold outline-none mb-6" value={newPerson.homeroom} onChange={e=>setNewPerson({...newPerson, homeroom: e.target.value})}><option value="">Sinf rahbarligi...</option>{classes.map(c=><option key={c.name} value={c.name}>{c.name}</option>)}</select><button onClick={handleAddTeacher} className="w-full py-4 bg-indigo-600 text-white rounded-2xl font-black">QO'SHISH</button></div></div>}
      {showClassModal && <div className="fixed inset-0 bg-slate-950/80 z-50 flex items-center justify-center p-4" onClick={() => setShowClassModal(false)}><div className="bg-white rounded-[3rem] w-full max-w-sm shadow-2xl p-8" onClick={e=>e.stopPropagation()}><h3 className="text-xl font-black mb-6">Yangi Sinf</h3><input type="text" placeholder="Nomi (9-A)" className="w-full p-4 bg-slate-100 rounded-2xl font-black text-center mb-4" value={newClassInfo.name} onChange={e=>setNewClassInfo({...newClassInfo, name: e.target.value})} /><input type="number" placeholder="Limit" className="w-full p-4 bg-slate-100 rounded-2xl font-bold text-center mb-6" value={newClassInfo.limit} onChange={e=>setNewClassInfo({...newClassInfo, limit: Number(e.target.value)})} /><button onClick={handleAddClass} className="w-full py-4 bg-slate-900 text-white rounded-2xl font-black">YARATISH</button></div></div>}
      {showStudentModal && <div className="fixed inset-0 bg-slate-950/80 z-50 flex items-center justify-center p-4" onClick={() => setShowStudentModal(false)}><div className="bg-white rounded-[3rem] w-full max-w-md shadow-2xl p-8" onClick={e=>e.stopPropagation()}><h3 className="text-xl font-black mb-6">Yangi O'quvchi</h3><input type="text" placeholder="F.I.SH" className="w-full p-4 bg-slate-100 rounded-2xl font-bold mb-6" value={newPerson.fullName} onChange={e=>setNewPerson({...newPerson, fullName: e.target.value})} /><button onClick={handleAddStudent} className="w-full py-4 bg-emerald-600 text-white rounded-2xl font-black">QO'SHISH</button></div></div>}
      {showEditTeacherModal && editingTeacher && <div className="fixed inset-0 bg-slate-950/80 z-50 flex items-center justify-center p-4" onClick={() => setShowEditTeacherModal(false)}><div className="bg-white rounded-[3rem] w-full max-w-md shadow-2xl p-8" onClick={e=>e.stopPropagation()}><h3 className="text-xl font-black mb-6">O'qituvchini Tahrirlash</h3><input type="text" className="w-full p-4 bg-slate-100 rounded-2xl font-bold mb-4" value={editingTeacher.full_name} onChange={e=>setEditingTeacher({...editingTeacher, full_name: e.target.value})} /><select className="w-full p-4 bg-slate-100 rounded-2xl font-bold mb-4" value={editingTeacher.bio} onChange={e=>setEditingTeacher({...editingTeacher, bio: e.target.value})}><option value="">Fan</option>{subjectsBase.map(s=><option key={s} value={s}>{s}</option>)}</select><select className="w-full p-4 bg-blue-50 text-blue-600 rounded-2xl font-bold mb-6" value={editingTeacher.homeroom || ""} onChange={e=>setEditingTeacher({...editingTeacher, homeroom: e.target.value})}><option value="">Sinf rahbarligi...</option><option value="">Yo'q</option>{classes.map(c=><option key={c.name} value={c.name}>{c.name}</option>)}</select><button onClick={handleUpdateTeacher} className="w-full py-4 bg-blue-600 text-white rounded-2xl font-black">SAQLASH</button></div></div>}
      {showEditStudentModal && editingStudent && <div className="fixed inset-0 bg-slate-950/80 z-50 flex items-center justify-center p-4" onClick={() => setShowEditStudentModal(false)}><div className="bg-white rounded-[3rem] w-full max-w-md shadow-2xl p-8" onClick={e=>e.stopPropagation()}><h3 className="text-xl font-black mb-6">O'quvchini Tahrirlash</h3><input type="text" className="w-full p-4 bg-slate-100 rounded-2xl font-bold mb-4" value={editingStudent.full_name} onChange={e=>setEditingStudent({...editingStudent, full_name: e.target.value})} /><select className="w-full p-4 bg-slate-100 rounded-2xl font-bold mb-6" value={editingStudent.class_name} onChange={e=>setEditingStudent({...editingStudent, class_name: e.target.value})}>{classes.map(c=><option key={c.name} value={c.name}>{c.name}</option>)}</select><button onClick={handleUpdateStudent} className="w-full py-4 bg-emerald-600 text-white rounded-2xl font-black">SAQLASH</button></div></div>}
