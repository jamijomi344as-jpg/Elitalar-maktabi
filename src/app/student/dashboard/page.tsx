...
import { BookOpen, CheckSquare, ChevronLeft, ChevronRight, Clock, FileText, GraduationCap, LayoutGrid, Square, User, Award, TrendingUp, CheckCircle2, Home, BookA, Wallet, Trophy, Info, LayoutDashboard } from "lucide-react";
...
export default function StudentDashboard() {
  ...
  // YON PANEL (SIDEBAR) MENYUSI: "asosiy" | "hamyon" | "talim" | "reyting"
  const [sidebarMenu, setSidebarMenu] = useState<"asosiy" | "hamyon" | "talim" | "reyting">("talim");
  ...
  return (
    <div className="flex h-screen bg-slate-50 font-sans overflow-hidden">
      ...
      {/* ======================================================== */}
      {/* YON PANEL (LEFT SIDEBAR) - 2-rasmdagi dizayn! */}
      {/* ======================================================== */}
      <div className="w-64 bg-white border-r border-slate-200 flex flex-col flex-shrink-0 z-20">
        ...
        <div className="p-4 flex flex-col gap-2 mt-2">
          ...
          <button 
            onClick={() => setSidebarMenu("asosiy")}
            className={`flex items-center px-4 py-3.5 rounded-xl font-bold text-[15px] transition-all ${sidebarMenu === 'asosiy' ? 'bg-slate-900 text-white shadow-md' : 'text-slate-500 hover:bg-slate-50 hover:text-slate-900'}`}
          >
            <LayoutDashboard className="w-5 h-5 mr-3" /> Asosiy
          </button>
          
          <button 
            onClick={() => setSidebarMenu("hamyon")}
            className={`flex items-center px-4 py-3.5 rounded-xl font-bold text-[15px] transition-all ${sidebarMenu === 'hamyon' ? 'bg-slate-900 text-white shadow-md' : 'text-slate-500 hover:bg-slate-50 hover:text-slate-900'}`}
          >
            <Wallet className="w-5 h-5 mr-3" /> Hamyon (PP)
          </button>

          {/* Dars Jadvali o'rniga TA'LIM */}
          <button 
            onClick={() => setSidebarMenu("talim")}
            className={`flex items-center px-4 py-3.5 rounded-xl font-bold text-[15px] transition-all ${sidebarMenu === 'talim' ? 'bg-slate-900 text-white shadow-md' : 'text-slate-500 hover:bg-slate-50 hover:text-slate-900'}`}
          >
            <BookA className="w-5 h-5 mr-3" /> Ta'lim
          </button>

          <button 
            onClick={() => setSidebarMenu("reyting")}
            className={`flex items-center px-4 py-3.5 rounded-xl font-bold text-[15px] transition-all ${sidebarMenu === 'reyting' ? 'bg-slate-900 text-white shadow-md' : 'text-slate-500 hover:bg-slate-50 hover:text-slate-900'}`}
          >
            <Trophy className="w-5 h-5 mr-3" /> Sinf Reytingi
          </button>
      ...
      <div className="flex-1 h-full overflow-y-auto p-6 md:p-8">
        <div className="max-w-6xl mx-auto space-y-6">
          ...
          <div className="bg-gradient-to-br from-blue-900 to-indigo-900 rounded-3xl p-8 text-white shadow-xl relative overflow-hidden animate-in fade-in zoom-in-95 duration-500">
            ...
            <div className="bg-white rounded-3xl shadow-sm border border-gray-100 overflow-hidden animate-in slide-in-from-bottom-4 duration-300">
              ...
              <div className="border-b border-gray-200 bg-slate-50 p-4 flex flex-wrap justify-center sm:justify-start gap-2">
                <button 
                  ...
                  className={`px-6 py-2.5 rounded-xl text-sm font-bold transition-all flex items-center ${activeTab === 'kundalik' ? 'bg-blue-100 text-blue-700 shadow-sm border border-blue-200' : 'bg-white text-gray-600 border border-gray-200 hover:bg-gray-50'}`}
                >
                  <BookOpen className="w-4 h-4 mr-2" /> Kundalik
                </button>
                <button 
                  onClick={() => setActiveTab("jadval")} 
                  className={`px-6 py-2.5 rounded-xl text-sm font-bold transition-all flex items-center ${activeTab === 'jadval' ? 'bg-blue-100 text-blue-700 shadow-sm border border-blue-200' : 'bg-white text-gray-600 border border-gray-200 hover:bg-gray-50'}`}
                >
                  <LayoutGrid className="w-4 h-4 mr-2" /> Dars jadvali
                </button>
                <button 
                  onClick={() => setActiveTab("vazifa")} 
                  className={`px-6 py-2.5 rounded-xl text-sm font-bold transition-all flex items-center ${activeTab === 'vazifa' ? 'bg-blue-100 text-blue-700 shadow-sm border border-blue-200' : 'bg-white text-gray-600 border border-gray-200 hover:bg-gray-50'}`}
                >
                    ...
                    <div className="flex flex-col md:flex-row justify-between items-center mb-6 bg-white p-4 rounded-2xl border border-gray-200 shadow-sm gap-4 md:gap-0">
                      ...
                      <div className="flex gap-1 bg-gray-100 p-1 rounded-xl border border-gray-200">
                        <button 
                          ...
                          className={`px-6 py-2 font-bold text-sm rounded-lg transition-all ${kundalikView === 'joriy' ? 'bg-blue-500 text-white shadow-md' : 'text-gray-600 hover:bg-gray-200'}`}
                        >
                          Joriy
                        </button>
                        <button 
                          onClick={() => setKundalikView("chorak")}
                          className={`px-6 py-2 font-bold text-sm rounded-lg transition-all ${kundalikView === 'chorak' ? 'bg-blue-500 text-white shadow-md' : 'text-gray-600 hover:bg-gray-200'}`}
                        >
                          Choraklar bo'yicha
                        </button>
                      </div>
