...
export default function EducationPage() {
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500 pb-10">
      ...
      <div className="bg-gradient-to-br from-blue-900 to-indigo-900 dark:from-slate-800 dark:to-slate-900 rounded-3xl p-8 text-white shadow-xl relative overflow-hidden animate-in fade-in zoom-in-95 duration-500 border border-transparent dark:border-slate-800">
      ...
      {/* ======================================================== */}
      {/* TA'LIM SAHIFASI KONTENTI */}
      {/* ======================================================== */}
      <div className="bg-white dark:bg-slate-900 rounded-3xl shadow-sm border border-gray-100 dark:border-slate-800 overflow-hidden animate-in slide-in-from-bottom-4 duration-300">
        ...
        <div className="border-b border-gray-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-800/50 p-4 flex flex-wrap justify-center sm:justify-start gap-2">
          <button 
            ...
            className={`px-6 py-2.5 rounded-xl text-sm font-bold transition-all flex items-center ${activeTab === 'kundalik' ? 'bg-blue-100 dark:bg-blue-900/50 text-blue-700 dark:text-blue-300 shadow-sm border border-blue-200 dark:border-blue-800' : 'bg-white dark:bg-slate-800 text-gray-600 dark:text-gray-400 border border-gray-200 dark:border-slate-700 hover:bg-gray-50 dark:hover:bg-slate-700'}`}
          >
            <BookOpen className="w-4 h-4 mr-2" /> Kundalik
          </button>
          <button 
            onClick={() => setActiveTab("jadval")} 
            className={`px-6 py-2.5 rounded-xl text-sm font-bold transition-all flex items-center ${activeTab === 'jadval' ? 'bg-blue-100 dark:bg-blue-900/50 text-blue-700 dark:text-blue-300 shadow-sm border border-blue-200 dark:border-blue-800' : 'bg-white dark:bg-slate-800 text-gray-600 dark:text-gray-400 border border-gray-200 dark:border-slate-700 hover:bg-gray-50 dark:hover:bg-slate-700'}`}
          >
            <LayoutGrid className="w-4 h-4 mr-2" /> Dars jadvali
          </button>
          <button 
            onClick={() => setActiveTab("vazifa")} 
            className={`px-6 py-2.5 rounded-xl text-sm font-bold transition-all flex items-center ${activeTab === 'vazifa' ? 'bg-blue-100 dark:bg-blue-900/50 text-blue-700 dark:text-blue-300 shadow-sm border border-blue-200 dark:border-blue-800' : 'bg-white dark:bg-slate-800 text-gray-600 dark:text-gray-400 border border-gray-200 dark:border-slate-700 hover:bg-gray-50 dark:hover:bg-slate-700'}`}
          >
              ...
              {kundalikView === "chorak" && (
                <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-gray-200 dark:border-slate-700 overflow-hidden animate-in slide-in-from-right-4">
                  <div className="p-4 border-b border-gray-200 dark:border-slate-700 flex flex-col md:flex-row justify-between items-center gap-4 bg-slate-50/50 dark:bg-slate-900/50">
                    ...
                    <div className="hidden lg:flex items-center bg-blue-50 dark:bg-blue-900/30 px-4 py-2 rounded-xl border border-blue-100 dark:border-blue-800 text-xs text-blue-600 dark:text-blue-300 font-medium">
                      ...
                      <tbody>
                        {quarterGrades.map((q, idx) => (
                          <tr key={q.id} className={`border-b border-gray-100 dark:border-slate-700 hover:bg-blue-50/20 dark:hover:bg-blue-900/20 transition-colors ${idx % 2 !== 0 ? 'bg-slate-50/30 dark:bg-slate-800/50' : 'bg-white dark:bg-slate-800'}`}>
                            ...
                            <td className="p-4 text-center font-black text-sm text-blue-600 dark:text-blue-400">
                              {q.total}
                            </td>
                          </tr>
                        ))}
          ...
          {activeTab === "vazifa" && (
            <div className="max-w-4xl mx-auto space-y-4 animate-in fade-in slide-in-from-right-4 duration-300">
              ...
              {sortedHomeworks.map((hw) => (
                ...
                >
                  <button 
                    ...
                    className={`mt-1 flex-shrink-0 transition-colors ${hw.completed ? 'text-green-500' : 'text-gray-300 dark:text-gray-500 hover:text-blue-500'}`}
                  >
                  ...
                  <div className="flex-1">
                    <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center mb-1 gap-2">
                      <h3 className={`font-bold text-lg ${hw.completed ? 'text-gray-500 dark:text-gray-400 line-through decoration-2' : 'text-blue-900 dark:text-blue-300'}`}>
                        {hw.subject}
                      </h3>
                    ...
                    <p className={`text-sm font-medium ${hw.completed ? 'text-gray-400 dark:text-gray-500 line-through' : 'text-gray-700 dark:text-gray-300'}`}>
                      {hw.text}
                    </p>
                  </div>
                </div>
              ...
              {homeworks.every(hw => hw.completed) && (
                <div className="bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 p-6 rounded-2xl flex flex-col items-center justify-center text-center mt-8 animate-in zoom-in-95">
                  <div className="w-16 h-16 bg-green-500 rounded-full flex items-center justify-center text-white mb-4 shadow-lg shadow-green-500/30 dark:shadow-none">
