...
import { User, GraduationCap, Award, TrendingUp, Info, BellRing, Calendar, Activity, Moon, Sun, Bell } from "lucide-react";

export default function DashboardPage() {
  const student = { id: "S-8392", name: "Kiyotaka Ayanokoji", class: "9-B", balancePP: 12000, cp: 150 };

  // ==========================================
  // QORA VA OQ FON (DARK MODE) FUNKSIYASI
  // ==========================================
  const [isDarkMode, setIsDarkMode] = useState(false);

  // Bu useEffect sahifa yuklanganda "dark mode" ni body'ga o'rnatadi yoki oladi
  useEffect(() => {
    if (isDarkMode) {
  ...
  return (
    <div className="w-full max-w-6xl mx-auto flex flex-col items-center space-y-6 animate-in fade-in duration-500 pb-10 relative">
      ...
      {/* TEPADAGI ASOSIY HEADER (Dark Mode va Notification tugmalari) */}
      <div className="w-full flex justify-between items-center bg-white dark:bg-slate-900 p-4 rounded-3xl shadow-sm border border-gray-100 dark:border-slate-800 transition-colors duration-300">
        <h2 className="font-black text-xl text-blue-900 dark:text-blue-400">O'quvchi Paneli</h2>
        
        <div className="flex items-center gap-3">
          {/* Tungi va Kunduzgi Rejim Tugmasi */}
          <button 
      ...
      {/* TEPADAGI PROFIL VA BALANS (Ko'k Kartochka) */}
      <div className="w-full bg-gradient-to-br from-blue-900 to-indigo-900 dark:from-slate-800 dark:to-slate-900 rounded-3xl p-8 text-white shadow-xl relative overflow-hidden animate-in fade-in zoom-in-95 duration-500 border border-transparent dark:border-slate-800 transition-colors duration-300">
      ...
      <div className="w-full grid grid-cols-1 lg:grid-cols-3 gap-6 animate-in slide-in-from-bottom-4 duration-300">
        ...
        <div className="bg-white dark:bg-slate-900 p-6 rounded-3xl shadow-sm border border-gray-100 dark:border-slate-800 lg:col-span-2 transition-colors duration-300">
          ...
          <div className="space-y-3">
            <div className="p-4 bg-indigo-50 dark:bg-indigo-900/20 rounded-2xl border border-indigo-100 dark:border-indigo-800 transition-colors duration-300">
            ...
            <div className="p-4 bg-slate-50 dark:bg-slate-800/50 rounded-2xl border border-slate-100 dark:border-slate-700 transition-colors duration-300">
        ...
        <div className="space-y-6">
          <div className="bg-white dark:bg-slate-900 p-6 rounded-3xl shadow-sm border border-gray-100 dark:border-slate-800 transition-colors duration-300">
            ...
            <ul className="space-y-2">
              <li className="flex justify-between items-center text-sm p-2 bg-slate-50 dark:bg-slate-800 rounded-lg transition-colors duration-300">
          ...
          <div className="bg-gradient-to-r from-emerald-500 to-teal-500 p-6 rounded-3xl shadow-sm text-white transition-colors duration-300">
